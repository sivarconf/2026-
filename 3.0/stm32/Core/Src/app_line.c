...
/* ============================ 速度/PID 参数配置 ============================ */
/* =====================================================
 *   基础题(B)参数组  —  高速（BASE=1300，跑完一圈声光提示，再跑完一圈停车）
 *   发挥题(F)参数组  —  低速保守（BASE=600，跑完一圈+K230识别后停车）
...
#define F_BASE_PWM   600
#define F_KP         60
#define F_KI         0
#define F_KD         40
/* ===================================================== */

/* 稳定模式参数（低速稳健，按键K1切换启用） */
static const LineParams_t s_stable_params[QUESTION_COUNT] = {
    {1000, 85, 5, 45},   /* B题稳定模式: base=1000, KP=8.5, KI=0.5, KD=4.5 */
    {630,  57, 0, 40}    /* F题稳定模式: base=630,  KP=5.7, KI=0.0, KD=4.0 */
};
...
/* 稳定模式标志：0=竞赛模式(C)，1=稳定模式(S) */
static uint8_t s_stable_mode = 0;
...
#define CARD_COVER_THRESH   (60U)    /* 60×5ms = 300ms，超过SHORT且<300ms视为扑克牌遮线，保持圆弧行进 */
...
void App_Line_Init(void)
{
    ...
    if (s_stable_mode) {
        params = s_stable_params[App_State_GetQuestion()];
    }
...
void App_Line_Task(void)
{
    ...
    if (s_status != LINE_LOST_LONG) {
        ...
        /* 发挥题(F)：一圈完成时通知K230停止识别 */
        if (q == QUESTION_F && !s_alert_done && s_enc_total >= target) {
            s_alert_done = 1;
            BSP_UART_K230_SendStop();
        }

        /* 达到目标圈数停车（F题一圈停车、B题两圈停车均触发） */
        if (s_enc_total >= target) {
            BSP_Beep_On();
            BSP_LED_On();
            BSP_Motor_Stop();
            s_alert_tick = HAL_GetTick();
            s_alert_off_done = 0;  /* 重置，确保本次声光能被自动关闭 */
...
{
    return s_stable_mode;
}

void App_Line_ToggleStableMode(void)
{
    s_stable_mode = s_stable_mode ? 0 : 1;
}
...
/* 停车后自动关闭声光提示（1秒后）
 * 由 user_main.c 主循环调用，确保状态切换后仍能执行 */
void App_Line_CheckAlertOff(void)
{
    if (s_alert_done && !s_alert_off_done) {
