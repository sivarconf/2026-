...
class StabilityStateMachine:
    """
    v16 算法（滑动窗口投票 + 永久冷却排除）：

    核心思路：
    - 滑动窗口投票：记录最近5帧识别结果，3帧同牌即确认
    ...
    关键改进：
    1. 滑动窗口替代严格连续帧：允许5帧中跳1-2帧，减少丢牌
    2. 永久冷却排除：冷却期结束后不清空 confirmed_cards/ranks
    3. 冷却期积累：冷却期内下一张牌可在 vote_window 中提前积累
    4. 参数：预热30帧 / 5帧窗口 / 3帧确认 / 冷却20帧
    """
...
def main():
    ...
    print("K230 Poker v16 | CONS={} CD={} | CONF={:.2f} BG={:.2f} WU={}".format(
